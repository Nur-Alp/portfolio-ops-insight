...
export function BrokerageCharts({ data, language, sourceRefs = [], includeRepo = true }: { data: ModuleReadResponse; language: Language; sourceRefs?: ProvenanceRef[]; includeRepo?: boolean }) {
  ...
  return <ChartGrid>
    <ChartCard title={l("Оборот покупок и продаж", "Buy and sell turnover")} subtitle={l(`${includeRepo ? "РЕПО включены" : "РЕПО исключены"}; суммы указаны в валюте каждой сделки, не являются эквивалентом KZT. Логарифмическая шкала показывает разные порядки величины.`, `${includeRepo ? "Repo included" : "Repo excluded"}; amounts are shown in each trade currency, not as KZT equivalents. The logarithmic scale keeps different orders of magnitude visible.`)} basis="source" sourceRefs={brokerageTradeRefs}>
    ...
    <ChartCard title={l("Количество сделок", "Trade count")} subtitle={l(`${includeRepo ? "РЕПО включены" : "РЕПО исключены"}; логарифмическая шкала делает малые валютные группы видимыми.`, `${includeRepo ? "Repo included" : "Repo excluded"}; a logarithmic scale keeps small currency groups visible.`)} basis="source" sourceRefs={brokerageTradeRefs}>
