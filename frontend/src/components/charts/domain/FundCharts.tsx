...
import { ChartCard, ChartEmpty, ChartGrid, ChartTooltip, Donut, COLORS, GRID, axisTick, compact, groupValues, numeric, shortDate, type Language, type ProvenanceRef, type Row } from "./shared";
...
export function FundCharts({ data, language, sourceRefs = [] }: { data: ModuleReadResponse; language: Language; sourceRefs?: ProvenanceRef[] }) {
  ...
  const history = (records.fund_unit_series ?? [])
    .filter((row) => row.record_type === "unit_observation" && numeric(row.unit_value_kzt) != null)
  ...
  return <ChartGrid>
    <ChartCard title={l("Стоимость пая", "Unit value")} subtitle={l("Источник; изменение между датами показано без заявления официальной доходности.", "Source-reported values; changes between dates are shown without claiming official performance.")} basis="source" sourceRefs={sourceRefs}>
    ...
    <ChartCard title={l("Состав портфеля фонда", "Fund portfolio allocation")} subtitle={l("По покупной стоимости опубликованных позиций.", "By purchase value of published holdings.")} basis="source" sourceRefs={sourceRefs}>
