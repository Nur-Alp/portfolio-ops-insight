...
export function OperationsPage() {
  ...
  return <PageFrame title={l("Операции и сверки", "Operations and reconciliations")} eyebrow="Portfolio Operations Insight / controlled sources" description={l("Рабочая очередь, независимые версии наборов и результаты межсистемных сверок.", "Work queue, independent dataset versions, and cross-source reconciliation results.")}>
    ...
    <Panel title={l("Межсистемные сверки", "Cross-source reconciliations")} subtitle={l(`Результаты автоматических проверок расхождений между независимо опубликованными источниками (правило, область, дата, разница, допуск, статус). Показано ${filteredReconciliations.length} из ${reconciliations.length} результатов; разные даты показываются как расхождение, а не объединяются автоматически.`, `Automated checks for differences between independently published sources (rule, scope, date, difference, tolerance, status). ${filteredReconciliations.length} of ${reconciliations.length} results shown; different dates are shown as a mismatch and are not merged automatically.`)}>
